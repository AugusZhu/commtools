...
public class MathExtendUtils {
    ...
    public static String getLeaseTerm(Object v1, Object v2) {
        String ret_val = "0";
        Double num_tmp = Double.parseDouble(v1.toString()) * Double.parseDouble(v2.toString());
        // 判断是否需要进位 12.0 11.999
        if (String.valueOf(num_tmp).lastIndexOf(".0") > 0) {
            ret_val = String
                    .valueOf(Integer.parseInt(String.valueOf(num_tmp).substring(0, String.valueOf(num_tmp).indexOf('.'))));

        } else if (String.valueOf(num_tmp).indexOf('.') > 0) {
            ret_val = String.valueOf(
                    Integer.parseInt(String.valueOf(num_tmp).substring(0, String.valueOf(num_tmp).indexOf('.'))) + 1);
        } else {
            ret_val = String
                    .valueOf(Integer.parseInt(String.valueOf(num_tmp).substring(0, String.valueOf(num_tmp).indexOf('.'))));
        }
